...
// --- 3. FUNÇÕES GLOBAIS (NECESSÁRIAS PARA OS BOTÕES DO HTML) ---
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (hasCache) {
            ...
            // B) SE NÃO TEM CACHE, MOSTRA O LOADING PARA O USUÁRIO NÃO FICAR PERDIDO
                ...
                list.innerHTML = `
                    <tr>
                        <td colspan="5" class="text-center py-10">
                            ...
                            <p class="text-slate-500 mt-2 font-bold">Acordando o servidor...</p>
                            <p class="text-xs text-slate-400">Isso pode levar até 1 minuto na primeira vez.</p>
                        </td>
                    </tr>
                `;
            }
        ...
        // C) Configura eventos (Botões, Abas) IMEDIATAMENTE
        setupEvents();

        // D) Busca atualização do servidor em background
        // (Isso demora ~50s se o servidor estiver dormindo, mas o usuário já vê o loading ou o cache)
        await store.init();
        
        // E) Verifica se o token ainda é válido
        if (!store.getToken()) {
            alert("Sessão finalizada. Faça login novamente.");
            window.location.href = 'login.html';
            return;
        }

        // F) Renderiza de novo com os dados frescos do servidor
        updateAllViews('Todos');
        ...
        const list = document.getElementById('transactionList');
        if(list) list.innerHTML = `<tr><td colspan="5" class="text-center py-10 text-red-500">Erro de conexão. Tente recarregar.</td></tr>`;
    }
});
...
function setupEvents() {
    ...
    const views = {
        ...
        debts: document.getElementById('viewDebts'),
    ...
    // --- ADICIONAR TRANSAÇÃO (COM LOADING NO BOTÃO) ---
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            // Feedback visual: Trava botão e mostra spinner
            ...
            try {
                // A store já trata atualização local + envio pro servidor
                ...
                console.error(err);
                // Erro já tratado no store.js (alert)
            } finally {
                // Restaura o botão
